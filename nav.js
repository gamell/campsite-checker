...
exports.run = async function run(page, info) {
  ...
  async function checkIfAvailable() {
    console.log(`Checking availability for campsiteId ${campsiteId}, site ${siteId}`);
    ...
    await page.waitForFunction((selector) => !document.querySelector(selector), { polling: 250 }, sel.loading);
    // close the pop over if it exists
    const maybeCloseButton = await page.evaluateHandle(getCloseOverlayButton, sel.popOverCloseButton);
    await maybeCloseButton.click();
    await page.waitFor(250);
    try {
      let siteInView = await page.evaluate(checkIfSiteInView, sel.sites, siteId);
      while(!siteInView) {
        ...
        await page.waitFor(siteRowsLargerThan, {}, sel.sites, initialRows);
        page.waitFor(100);
        siteInView = await page.evaluate(checkIfSiteInView, sel.sites, siteId);
      }
    } catch(e) {
      logError(`Tried to load more campsites until the desired campsite was in view but failed`);
      throw new Error(e);
    }
    // get the handle for the start day
    startDayHandle = await page.evaluateHandle(getSiteAvailableHandle, sel.sites, siteId, tablePadding);
  ...
  async function checkUntilAvailable(){
    ...
    while(!isAvailable) {
      console.log(`Campsite not available yet. Trying again in ${WAIT_AVAILABLE/1000/60} minutes...`);
  ...
  async function selectTrip({ clickBookNow } = { clickBookNow: true }) {
    ...
    if (lengthOfStay > 1) {
      ...
      const endDayHandle = await page.evaluateHandle((selector, siteId, column) => {
  ...
  async function checkIfInCheckout(maybeError) {
    ...
    await page.waitForFunction(waitForUrlToContain, 'reservations/orderdetails');
  ...
  async function parseBookableTime() {
    ...
    const text = await page.evaluate(getTextFromSelector, bookingErrorMessage);
  ...
  async function maybeParseBookableTime() {
    ...
    return page.waitFor(sel.bookingErrorIcon, { timeout: 20000 }) // 20s timeout
  ...
  async function maybeGetBookableTimeAndAwait() {
    ...
    const safeSetupTime = bookableTime.clone().subtract(MINUTES_TO_GET_READY + minutesRefresh + 1, 'minutes');
    ...
    while (moment().isBefore(safeSetupTime)) {
      ...
      page.reload();
      console.log(`Waiting ${WAIT_REFRESH_BOOKABLE/60/1000} minutes to refresh again`);
      console.log(`${moment.duration(bookableTime.diff(moment())).asMinutes()} minutes left to bookableTime`);
      await sleep(WAIT_REFRESH_BOOKABLE);
    }
    console.log(`Finished refreshing, now will wait for setupTime! (${MINUTES_TO_GET_READY}mins before bookableTime)`);
  }
  ...
  async function waitUntilSetupTime() {
    ...
    while(setupTime.isBefore(moment())) {
      ...
      console.log(`Waiting for setupTime: ${moment.duration(setupTime.diff(moment())).asMinutes()}min remaining until ${setupTime.format()}`);
    }
    console.log(`Finished waiting for setupTime!, now will setup everything and wait to push the book button`);
  }
  ...
  async function waitUntilBookableTimeAndBook() {
    ...
    console.log(`${moment().format('HH:mm:ss:sss')} clickBookNowTime! CLICKING "BOOK NOW" BUTTON!`);
    await clickBookNowButton(); 
  }
